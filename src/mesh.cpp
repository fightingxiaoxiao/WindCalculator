...
#pragma omp parallel for
        ...
        {
            //Point &dataPoint = &(cloud->storage[p_i]);
            ...
            label i_max = ((i_mid + 1) > I - 1) ? I - 1 : i_mid + 1;
            label j_max = ((j_mid + 1) > J - 1) ? J - 1 : j_mid + 1;
            label k_max = ((k_mid + 1) > K - 1) ? K - 1 : k_mid + 1;
...
#pragma omp critical
                              ...
                              Map<String, label> field_index, bool hasHeader)
    {
        ...
        if (!_file)
        {
            std::cout << "Error read point data.\n";
            return EXIT_FAILURE;
        }
        ...
        while ((row = CsvParser_getRow(csvparser)))
        {
            ...
            {
                //std::cout << field.first << ", " << rowFields[field.second] << std::endl;

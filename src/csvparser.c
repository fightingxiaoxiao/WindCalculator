...
extern "C"
{
    ...
    CsvParser *CsvParser_new(const char *filePath, const char *delimiter,
    ...
    CsvParser *CsvParser_new_from_string(const char *csvString,
                                         const char *delimiter,
                                         int firstLineIsHeader)
    {
        CsvParser *csvParser =
            CsvParser_new(NULL, delimiter, firstLineIsHeader);
    ...
    const CsvRow *CsvParser_getHeader(CsvParser *csvParser)
    {
        if (!csvParser->firstLineIsHeader_)
        {
            _CsvParser_setErrorMessage(
                csvParser, "Cannot supply header, as current CsvParser object "
    ...
    CsvRow *_CsvParser_getRow(CsvParser *csvParser)
    {
        ...
        while (1)
        {
            ...
            if (isEndOfFile ||
                ((currChar == csvParser->delimiter_ || currChar == '\n') &&
                 !inside_complex_field))
            {
                currField[lastCharIsQuote ? currFieldCharIter - 1
                                          : currFieldCharIter] = '\0';
                csvRow->fields_[fieldIter] =
                    (char *)malloc(currFieldCharIter + 1);
                ...
                if (csvRow->numOfFields_ != 0 &&
                    csvRow->numOfFields_ % acceptedFields == 0)
                {
                    csvRow->fields_ =
                        (char **)realloc(csvRow->fields_, ((numRowRealloc + 2) *
                                                           acceptedFields) *
                                                              sizeof(char *));
                    numRowRealloc++;
                }
            ...
            else
            {
                ...
                if (currFieldCharIter == acceptedCharsInField - 1)
                {
                    ...
                    currField =
                        (char *)realloc(currField, acceptedCharsInField);
                }
            }
    ...
    int _CsvParser_delimiterIsAccepted(const char *delimiter)
    {
        ...
        if (actualDelimiter == '\n' || actualDelimiter == '\r' ||
            actualDelimiter == '\0' || actualDelimiter == '\"')
        {
            return 0;
        }
        return 1;
